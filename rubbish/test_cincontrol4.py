...
class Scan(object):
    ...
    def save_tifs(self):
        ...
        fastpath = self.dark_dir_ram + self.scheme
        nummer =0 
        for i in range(self.dark_max):
            ...
            if self.dark_full and d is None:
                ...
                nummer = self.to_tif(d, 0, path, fastpath)
        ...
        fastpath = self.exp_dir_ram + self.scheme
        for i in range(self.exp_max):
            ...
            if self.exp_full and d is None:
                ...
                nummer = self.to_tif(d, nummer, path, fastpath)
...
class Grabber(object):
    ...
    def zmq_receive(self,socket_address=None,pub_address=None):
        addr = 'tcp://%s' % self.pars['framegrabber']['pub_addr']
        self.frame_socket.connect(addr)
        self.status = "Start reading frames"
        while not self.scan.exp_full:
            number, frame = self.frame_socket.recv_multipart()
            self.fbuffer.append((int(number), frame))
        self.status = "Stooped reading frames"
        self.frame_socket.disconnect(addr)
    ...
    def on_finished_scan(self):
        ...
        #self.scan.save_tifs()
        try:
            self.statusterm.write(term['blue']("%d frames in buffer" % len(self.fbuffer)) +'\n')
        except:
            pass
    ...
    def on_start_capture(self, msg):
        ...
        if self.scan_is_dark:
            self.fbuffer = scan.dark_frames
            scan.dark_dir =  self.save_dir
        else:
            scan.dark_full = True
            scan.exp_dir =  self.save_dir
            scan.save_json()
            self.fbuffer = scan.exp_frames
...
"""
                ...
                index = cmd.index(" ")
                resp_command = cmd[0:index].strip()
                exp = float(cmd[index+1:].strip())
                self.cin.setExpTime(exp)
                
            elif "setExp2" in resp_command:
                index = cmd.index(" ")
                resp_command = cmd[0:index].strip()
                exp = double(cmd[index+1:].strip())
